...
**Status:** 2.0.0 shipped on `main` (`039b7aa`). Active development on `feature/generator-v2`: the **encoder refactor (2.1 line)** is mid-W5. The `gen2` package (`skills/voiceattack-generator/scripts/gen2/`, stdlib-only, dictionary-driven) consumes schema-v1.1 JSON and lowers the simple authoring format through one emission pipeline, with the overloaded-trigger idiom auto-lowered to a `{LASTSPOKENCMD}` dispatch chain (collision-checked by exhaustive utterance simulation). Row-1 types plus SetDecimal/Write/conditionals emit and are import-proven; W5 wired row-2 (TextSet/BooleanSet/IntSet/FreeType) — VA-import-unverified pending Richard's session (`Row2 Probe.vap` + the dark-type XML export per the W5 build sheet). The `decode(encode(decode(x)))==decode(x)` fixpoint is a live repo-level gate (`tests/integration/`). Decoder V2 (`scripts/vap2/`, stdlib-only object-walk) decodes binary and XML-form profiles into the same normative JSON (schema v1.1, frozen), structured conditionals included; all reference profiles decode with zero chain breaks; unknown-marked actions are zero everywhere except base profile (14, codes 4/34/57/69/70/86 — dark types outside dictionary scope, honestly marked). v1 tools (`vap_decoder.py`, `vap_generator.py`) stay in-tree during soak. Harness: 15 integration + 54 decoder tests green; dictionary 0.4.1 (audit exit 0, but the audit scans the v1 generator only — it does not yet certify `gen2`; W6 owns that). Next: W5 closeout (dark-type ground truth) → W6 (runtime dictionary loading, audit fix) → W7 (docs, 2.1.0 release).
