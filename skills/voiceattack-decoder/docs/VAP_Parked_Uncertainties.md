...
**XML-layer update (2026-07-11).** A worked example now exists at the XML layer: mandiant's `IDA_Pro_VoiceAttack_profile` command "go down a lot" exports a compound `ConditionStart` (`[speed] Equals 0 OR [speed] Equals 1`) with its top-level `ConditionStartNameFrom`/`ConditionStartValue` zeroed and the two sub-compares carried in a populated `<ConditionExpressions><ArrayOfConditionStruct><ConditionStruct>…</ConditionStruct></ArrayOfConditionStruct>…` block (one `ArrayOfConditionStruct` per sub-compare, each `ConditionStruct` holding its own `ConditionStartType`/`ConditionStartNameFrom`/`ConditionStartValueType`/`ConditionStartValue`/`ConditionStartOperator`). This confirms the XML layer distinguishes compound from simple compares structurally (populated `ConditionExpressions` vs. empty) but the record format itself — sub-compare field semantics beyond the inline names already known, and the AND/OR conjunction marker — stays PARKED. Unlike the binary path (§8.6), the XML decoder does not yet read `ConditionExpressions` or emit a `compound` marker for it: a compound Begin's zeroed top-level inline fields decode without error (no crash, per the XML-input absent-element contract) but without the binary path's explicit compound-count marker — a known XML/binary parity gap, tracked here rather than silently claimed as closed.
