...
| m[12] | u32 | polymorphic | Say rate (7 observed); mouse Move Y (444 observed); PressKey variable-duration Y flag (=1 gates the m[15] variable name — VA binary re-exports 2026-08-15, 12 flagged / 62 unflagged presses, zero exceptions); also nonzero (=1) alongside m[11] on Set-Integer arithmetic-mode actions (siv6–siv11) — meaning of that secondary flag not isolated, park as minor open item | [SOLID for Say/Move/PressKey flag; OPEN for the Set-Integer arithmetic flag] |
...
| m[15] | string | Set target name (Integer, Decimal); PressKey variable-duration variable name | disjoint from m[6]'s Text/Bool targets; Probe B: `'siv0'`..`'siv11'`, `'ssi'`, `'sd'`; on PressKey, read ONLY when the m[12] Y flag =1 (0xFFFFFFFF on ordinary presses; `'k'`/`'ang'`/`'pan'`/`'z'` observed, VA binary re-exports 2026-08-15) | [SOLID] |
