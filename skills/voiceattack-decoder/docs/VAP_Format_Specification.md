...
Status: reverse-engineered, consolidated. **Version 0.2, 2026-07-09.** Author: analysis sessions over the profiles in §2. This version folds three documents into one authoritative spec: v0.1 (2026-07-08), the independent verification review (`VAP_Format_Specification_Review.md` — 1,601-object walk, nine corrections §2.1–2.9 applied here), and the Phase 0–2 uncertainty closures (`VAP_Uncertainty_Closure_Table.md` — 1,603-object slot census, 12 verdicts). It is not derived from VoiceAttack source; every non-trivial claim cites the bytes or the evidence doc it rests on.

**Precedence.** This document alone is authoritative. The Review and Closure Table are folded in and retained as the historical evidence trail; where any document conflicts with this one, this one wins. `VAP_FORMAT.md` and `VAP_Binary_Schema_Analysis.md` are superseded flat-scan-era notes; `VAP_Conditional_Command_Analysis.md` is the session history. v0.1's five refuted claims (Decimal-as-double, m[7]-as-universal-right-operand, m[5]-as-marker, m[27]/m[28]=0xF1886E09, three-GUIDs-per-command) are corrected in place in this version.
...
Census totals: 1,603 action objects across the five profiles; every one chains cleanly with m[0]=32 and m[1]=140, zero structural exceptions. [SOLID] (The review's 1,601 / corinthian-1,166 differs by a two-object harness mismatch; the count-driven walk's 1,168 is authoritative — anchored by set-fire=37 and the profile-record command count=201, both reproduced.)
...
| m[1] | GUID | action Id (all) | per-action GUID at heap start; 1,603 distinct across census | [SOLID] |
| m[2] | u32 | **`ActionType`** (all) | integer code; full enum §9.1; census sees 40 distinct codes | [SOLID] |
| m[3] | double | `Duration` (keypress, Pause) | keypress hold time; Pause seconds (CSV: 1.125 reproduced) | [SOLID] |
| m[5] | list | **`KeyCodes`** (key actions) | `[u32 count][count × u16 VK]`; chords read count 2–3 ([162,164,69] = LCtrl+LAlt+E) | [SOLID — review §2.4] |
| m[6] | string | polymorphic | Set-action target variable name (Text, Boolean); mouse context code (§9.3) | [SOLID] |
| m[7] | string | polymorphic | `ConditionStartValue` for **Text compares only** (`out`=3, `in`=2); Set-Text value; mouse parameter | [SOLID] |
| m[11] | i32 | Set-Integer literal | climb's 99; 58/58 corinthian Set-Integers verified | [SOLID] |
| m[14] | u32 | Set-Boolean value | 0=True / 1=False, positional-dropdown coding; value-vs-order confound not yet eliminated | [PLAUSIBLE → Probe B] |
| m[15] | string | Set target name (Integer, Decimal) | disjoint from m[6]'s Text/Bool targets | [SOLID] |
...
| m[19] | string | left operand (compares) | inline token (`{LASTSPOKENCMD}`, `{TXT:bbq}`) OR pool-local variable name — same slot both ways | [SOLID] |
| m[20] | u32 | polymorphic | `ConditionStartOperator` on compares (§8.2); set-operation code on Set actions — enum unresolved (§12) | [SOLID / OPEN] |
...
| m[23] | u32 | — | binary flag, ≡0xFFFFFFFF (1,359 objects) or 0 (244); meaning unmapped | [OPEN] |
| m[24] | u32 | `ConditionStartValueType` | 0=Small Int, 1=Text, 2=Bool, 3=Int, 4=Decimal (§8.1) | [SOLID] |
| m[25] | Decimal 16 B | Decimal value | Decimal compare value AND Set-Decimal value (.NET layout, §4) | [SOLID — review §2.1] |
| m[27], m[28] | u32 | structural constant | deref = **0x886E0900**; byte layout `[00][09 6E 88 F1][FF FF FF FF]` — the recurring 4-byte constant 0xF1886E09 sits at deref+1. An equality test against 0xF1886E09 at deref FAILS | [SOLID — review §2.7] |
...
Gotcha: m[20] is a SHARED slot — on a `Set integer` action it reads the set-operation code, not an operator, and the set-op enum is NOT yet trustworthy (climb "converted value"→4 but jumped "converted value"→0 — contradictory; Probe B sweeps the dropdown). **Gate operator reads on the ActionType, m[2]: treat an action as a compare only when m[2] ∈ {19 Begin, 63 Else If, 30 Begin Loop While}.** Do NOT gate on m[24] — Small-Integer conditions AND every non-condition action read m[24]=0, so it filters nothing; m[24] is DESCRIPTIVE (the value-type), not a condition flag. [SOLID]
...
The ActionType is member m[2] — an integer code per action type, mapped by correlating walks against CSV/UI action names. The census observes **40 distinct codes** across the five profiles; the attributed ones:

| code | ActionType | code | ActionType |
|------|-----------|------|-----------|
| 0 | PressKey | 30 | Begin Loop While (compare) |
| 2 | Pause | 31 | End Loop |
| 8 | KeyDown | 32 | Marker |
| 9 | KeyUp | 33 | Jump to Marker |
| 12 | MouseAction | 35 | Play Sound |
| 16 | Execute Command | 36 | Set Boolean |
| 17 | Kill Command | 37 | Set Integer |
| 18 | Set Small Int | 38 | Set Decimal |
| 19 | Begin Condition (compare) | 40 | Quick Input |
| 20 | End Condition | 62 | Pause Variable |
| 21 | Set Text | 63 | Else If (compare) |
| 22 | Execute External Plugin | 64 | Exit Command |
| 23 | Write | 67 | KeyToggle |
| 29 | Else | | |

All CSV-confirmed [SOLID]. PLAUSIBLE (single-action command whose name is its sole action): 24 paste dictation · 25 dictation mode · 26 stop dictation · 27 clear dictation buffer. Known-unlabeled: **50, 51** (start/stop listening) → Probe B. Codes 19 / 63 / 30 carry a compare (read operator m[20], type m[24], operands); 20 / 29 / 31 are block-structure actions with no operand. 62 (Pause Variable, `[{DEC:x}]` seconds) is distinct from fixed Pause 2. **KeyDown / KeyUp / KeyToggle are distinct ActionTypes (8 / 9 / 67), not a subtype member** — closure item 2. Unattributed census codes and Say / Launch / SetClipboard remain unsampled [OPEN → Probe B]. This SUPERSEDES the flat-scan note that "`01 00 00 00` = ActionType 1 = PressKey": that `01` is m[5]'s KeyCodes count=1; PressKey's code is 0. [SOLID]
...
### 9.3 Mouse actions  [PARTIAL — walk-verified slots; enum and X/Y → Probe B]

MouseAction = m[2] code 12. The context-code string lives at **m[6]** and the action parameter at **m[7]** (walk-verified; the polymorphism warning in §6.4 applies). Context code is `{button}{action}` (e.g. `LC`, `RDC`, `SF`). The flat-scan-era claims (scroll-click count as a double at −20 from the context-code length prefix; XML maps scroll count to `<Duration>`) have NOT been object-walk verified and keep [INFERRED]. The full context-code enum, click-count slot confirmation, and cursor-move X/Y are Probe B targets. [SOLID for m[2]/m[6]/m[7]]

### 9.4 Set actions and others

**Set-action field map — disjoint from the condition map, per-type** [SOLID, closure item 5]:

| Set type (m[2]) | value slot | target-name slot |
|-----------------|-----------|------------------|
| Set Text (21) | m[7] string | m[6] |
| Set Boolean (36) | m[14] u32, 0=True/1=False [PLAUSIBLE → Probe B] | m[6] |
| Set Integer (37) | m[11] i32 (58/58 verified) | m[15] |
| Set Decimal (38) | m[25] .NET Decimal | m[15] |
| Set Small Int (18) | layout unsampled [OPEN → Probe B] | — |

The Set-operation code shares m[20] with the condition operator; its enum is unresolved (contradictory samples) → Probe B dropdown sweep. `Launch` (executable path, `*` prefix = window-title match), `Say` (never sampled — absent from every CSV), `SetClipboard`, and dictation actions have no confirmed codes or layouts beyond the shared envelope [OPEN → Probe B].
...
Closed [SOLID]: container/compression; primitives incl. GUID endianness and the 16-byte .NET Decimal; header fields @0/@4/@0x170+ and offset-table entries 0–2; command envelope and actionCount; the action-object envelope (fixed 34 members, offset-deref rule, head chaining, fixed heap-header offsets, m[0]=32, m[1]=GUID); 27 ActionType codes incl. KeyDown/Up/Toggle as distinct types; the condition system (value-type codes, all four operator dropdowns, per-type operand slots m[7]/m[21]/m[25], pairing semantics generalized to all block actions, m[18] ordinal, IndentLevel derived-not-stored); Set-action field map (Text/Int/Dec + targets); Pause duration m[3]; keypress Duration/KeyCodes slots; the structural constants; all seven hazards.

Open, routed:

| # | item | state | route |
|---|------|-------|-------|
| 1 | ActionTypes 50/51; Say / Launch / SetClipboard / dictation codes + layouts | unlabeled/unsampled | **Probe B** |
| 2 | Mouse context-code enum, click-count slot, cursor X/Y | m[2]/m[6]/m[7] known | **Probe B** |
| 3 | Set-op m[20] enum | contradictory samples | **Probe B** (dropdown sweep) |
| 4 | Set-Boolean m[14] value-vs-order confound | PLAUSIBLE (0=True) | **Probe B** |
| 5 | Set-Small-Int layout | unsampled | **Probe B** |
...
| 8 | ~24 near-constant member slots; m[23] FFFF/0 flag; m[0]=32 meaning; m[27..28] 9-byte structure | unmapped constants | **PARKED** |
...
| 11 | Unattributed census ActionType codes (40 observed vs 33 attributed) | inventory only | **Probe B** / parked residue |
